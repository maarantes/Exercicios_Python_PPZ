...
for palavra in TextoA:
    if palavra[0] in zumbi and palavra[-1] not in zumbi:
        preposicaoA.append(palavra)

print ("Preposições no Texto A =" , len(preposicaoA))

for palavra in TextoB:
    if palavra[0] in zumbi and palavra[-1] not in zumbi:
        preposicaoB.append(palavra)

print ("Preposições no Texto B =" , len(preposicaoB))

# Verbos e Verbos em primeira pessoa

for palavra in TextoB:
    if len(palavra) == 7 and palavra[-1] not in zumbi:
        verbos.append(palavra)
    if len(palavra) == 7 and palavra [0] not in zumbi and palavra[-1] not in zumbi:
        verbospessoa.append(palavra)

print ("Verbos no Texto B =" , len(verbos))
print ("Verbos em primeira pessoa no Texto B =" , len(verbospessoa))
...
def ordem_diff(x):
    ...
    resposta = ""
    for letra in x:
        posicao = ordem.find(letra)
        resposta = resposta + alfabeto[posicao]
    return resposta

TextoB.sort(key=ordem_diff)
print (TextoB)
...
def valor(x):
    ...
    soma = 0
    for letra in x:
        soma = soma + ordem.find(letra) * base20
        base20 = base20 * 20
    return soma

def magico(x):
    if valor(x) % 42 == 0 and len(x) == len(set(x)):
        return x

for palavra in TextoB:
    if magico(palavra):
        num_magico.append(palavra)

print ("Palavras Mágicas:" , num_magico)