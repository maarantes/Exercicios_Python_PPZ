...
def not_bad(s):
  parte_not = s.find("not")
  parte_bad = s.find("bad")
  if parte_bad > parte_not:
    s = s[:parte_not] + "good" + s[parte_bad+3:]
  return s
...
def inip2(n):
  potencia = 0
  while True:
    potencia_resultado = str(2**potencia)
    if potencia_resultado.startswith(str(n)):
      break
    else:
      potencia = potencia + 1
  return potencia
